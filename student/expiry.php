                ...
                <div class="container book-req-container">
                    ...
                    if(isset($_SESSION['login_user'])){
                        ...
                        if(isset($_POST['submitRet'])){
                            ...
                            WHERE issue_book.approve = '$ret' AND issue_book.userName = '$_SESSION[login_user]' ORDER BY returnDate DESC";
                            $res = mysqli_query($db,$sql);
                        }
                        else if(isset($_POST['submitExp'])){
                            $sql = "SELECT `student`.userName,class,year,roll,books.b_id,name,authors,edition,approve,issue_book.issue,returnDate
                            FROM `student` inner join issue_book ON student.userName=issue_book.userName inner join books ON issue_book.b_id=books.b_id 
                            WHERE issue_book.approve = '$exp' AND issue_book.userName = '$_SESSION[login_user]' ORDER BY returnDate DESC";
                            $res = mysqli_query($db,$sql);
                        }
                        else if(isset($_POST['submitAll'])){
                            $sql = "SELECT `student`.userName,class,year,roll,books.b_id,name,authors,edition,approve,issue_book.issue,returnDate
                            FROM `student` inner join issue_book ON student.userName=issue_book.userName inner join books ON issue_book.b_id=books.b_id 
                            WHERE issue_book.userName = '$_SESSION[login_user]' AND issue_book.approve !='Yes' AND issue_book.approve != '' ORDER BY returnDate DESC";
                            $res = mysqli_query($db,$sql);
                        }
                        else{
                            $sql = "SELECT `student`.userName,class,year,roll,books.b_id,name,authors,edition,approve,issue_book.issue,returnDate
